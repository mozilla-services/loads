...
class TestTestResult(TestCase):
    ...
    def _get_data(self, url='http://notmyidea.org', method='GET',
                  status=200, started=None, elapsed=None, hit=1, user=1,
                  current_hit=1, current_user=1):
        started = started or TIME1
        loads_status = (hit, user, current_hit, current_user)
    ...
    def test_average_request_time_with_hit_filtering(self):
        test_result = TestResult()
        test_result.add_hit(**self._get_data(elapsed=_1, hit=1))
        test_result.add_hit(**self._get_data(elapsed=_3, hit=2))
        test_result.add_hit(**self._get_data(elapsed=_2, hit=3))
        test_result.add_hit(**self._get_data(elapsed=_3, hit=3))

        avg = test_result.average_request_time(hit=3)
        ...
        self.assertEquals(test_result.average_request_time(hit=3),
                          2.6666666666666665)
    ...
    def test_hits_success_rate(self):
        ...
        test_result.add_hit(**self._get_data(status=400, hit=2))

        self.assertEquals(test_result.hits_success_rate(), 0.8)
        self.assertEquals(test_result.hits_success_rate(hit=1), 1)
    ...
    def test_get_tests_filters_hits(self):
        test_result = TestResult()

        test_result.tests['bacon', 1] = Test(name='bacon', hit=1)
        test_result.tests['egg', 1] = Test(name='egg', hit=1)
        test_result.tests['spam', 2] = Test(name='spam', hit=2)

        self.assertEquals(len(test_result._get_tests(hit=1)), 2)

    def test_get_tests_filters_names(self):
        test_result = TestResult()

        test_result.tests['bacon', 1] = Test(name='bacon', hit=1)
        test_result.tests['bacon', 2] = Test(name='bacon', hit=2)
        test_result.tests['spam', 2] = Test(name='spam', hit=2)
    ...
    def test_get_tests_filters_by_both_fields(self):
        ...
        self.assertEquals(len(test_result._get_tests(name='bacon', hit=2)),
                          1)
